...
router.post('/signInWithPassword', [
  ...
  async (req, res) => {
    try {
      ...
      const isPassswordEqual = await bcrypt.compare(password, existingUser.password)
      if (!isPassswordEqual) {
        return res.status(400).send({
          error: {
...
router.post('/token', async (req, res) => {
  ...
  try {
    const { refresh_token: refreshToken, role } = req.body
    ...
    res.status(200).send({ ...tokens, userId: data._id, role })
  } catch (e) {
    res.status(500).json({
      message: 'На сервере произошла ошибка. Попробуйте позже',
    })
